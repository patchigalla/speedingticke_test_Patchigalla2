...
class MathsQuiz:
  ...
  def __init__(self,parent):
    ...
    tkvar.set(7) #sets 7 as default age

    self.age_drop_down = OptionMenu(self.Welcome, tkvar, *age_range)    
    self.age_drop_down.grid(row=3, column=1)
    self.age_drop_down.config(width=15)
...
#Error checking for empty or non-text user entries for Name
        if int(self.AgeEntry.get()) < 13 or int(self.AgeEntry.get()) > 18:
          self.WarningLabel.configure(text = "You are not eligible")
          self.AgeEntry.focus()
...
#Error checking for empty and age limit cases
          ...
          self.WarningLabel.configure(text = "Use a positive number")
          self.AgeEntry.delete(0, END)
          self.AgeEntry.focus()
