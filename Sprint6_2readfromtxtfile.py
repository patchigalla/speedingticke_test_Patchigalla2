...
class MathsQuiz:
  ...
  def __init__(self, parent):
    ...
    """Widgets for Report_frame with treeview widget"""
    self.Report_frame = Frame(parent)
    ...
    self.DataHomeButton = ttk.Button(self.Report_frame, text='Home', command=self.Restart)
  ...
  def next_question(self):
    ...
    if self.index >= 6:
      ...
      with open('records.txt', 'a+') as data_file:
        data_file.write(self.name.get() + ' ' + str(self.age.get()) + ' ' + str(self.score) + ' '+ str(datetime.now()) + '\n')
