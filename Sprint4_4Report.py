...
class MathsQuiz:
    def __init__(self, parent):
        ...
        self.diff_lvl.set(
            0)  # .set is used so that when the radio buttons appear it is automatically set on the easy difficuty,
        self.diff_btns = []

        """The difficulty levels are stored in a list and the radio button selected corresponds to the a number in the 
      list, such as "easy" """
        ...
        for i in range(len(Report_page)):
            ColumnHeading = Label(self.Report_frame, text=Report_page[i], width="7", height="2",
                                  font=("Times", "22", "bold"))
    ...
    def show_Questions(self):
        ...
        try:
            ...
            elif not self.NameEntry.get().isalpha():
